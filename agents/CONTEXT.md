Overall Context: Dependency-free departure board with Entur APIs, intelligent autocomplete, Norwegian support. Versioned service worker caching. Options panel with transport modes, multilingual (8 languages), station dropdown (max 5 recent). CSS refactored for dark/light mode: max 10% transparency, white text, visible white glow shadows on destinations/icons/countdowns.
