Overall Context: Dependency-free departure board with Entur APIs, intelligent autocomplete, Norwegian support. Versioned service worker with automatic update reloading and version validation tests. Options panel with transport modes, multilingual (11 languages: Norwegian, Deutsch, English, Español, Français, Ελληνικά, हिन्दी, Íslenska, Italiano, فارسی, Українська), station dropdown (max 5 recent). CSS refactored for dark/light mode: max 10% transparency, white text, unified button styles (including gear button) with consistent backgrounds and borders.
